...
void Engine::Graphics::D3D11Renderer::RenderQuad(Engine::Resources::MeshObject quad)
{
    ...
    Engine::Math::Matrix4x4 objectMatrix =
        Engine::Math::Matrix4x4::FromTranslation(quad.transform.position)
        * Engine::Math::Matrix4x4::FromOrientation(quad.transform.rotation)
        * Engine::Math::Matrix4x4::FromScale(quad.transform.scale);
    ...
    immediateContext->IASetVertexBuffers(0, 1, &quad.vertexBuffer, &stride, &offset);
    immediateContext->IASetIndexBuffer(quad.indexBuffer, DXGI_FORMAT::DXGI_FORMAT_R32_UINT, 0);
    ...
    //Pixel Shader Stage
    immediateContext->PSSetShaderResources(0, 1,& quad.resourceView);
    immediateContext->PSSetSamplers(0, 1,& quad.sampler);
    immediateContext->PSSetShader(textureShader, nullptr, 0);
    ...
    immediateContext->DrawIndexed(quad.indexCount, 0, 0);
}

void Engine::Graphics::D3D11Renderer::RenderCube(Engine::Math::Quaternion rotation, Engine::Resources::MeshObject cube)
{
    //Input Assembler Stage
    ui32 stride = sizeof(Engine::Graphics::Vertex);
    ui32 offset = 0;
    ...
    immediateContext->IASetVertexBuffers(0, 1, &cube.vertexBuffer, &stride, &offset);
    immediateContext->IASetIndexBuffer(cube.indexBuffer, DXGI_FORMAT::DXGI_FORMAT_R32_UINT, 0);
...
void Engine::Graphics::D3D11Renderer::CreateShader()
{
    ...
    ID3DBlob* textureShaderBlob = nullptr;
    if (FAILED(D3DReadFileToBlob(L"./data/shd/textureDefault.shader", &textureShaderBlob)))
    {
        printf("Caught Error: D3DReadFileToBlob(texture) FAILED!\n");
        return;
    }
    ...
    {
        printf("Caught Error: CreatePixelShader() FAILED!\n");
        return;
    }

    if (FAILED(device->CreatePixelShader(textureShaderBlob->GetBufferPointer(), textureShaderBlob->GetBufferSize(), nullptr, &textureShader)))
    {
        printf("Caught Error: CreateTextureShader() FAILED!\n");
        return;
    }
